...
def main(imgs, cluster_name, detection_image='rimg', use_megacat=True,
    ...
    bkg_imgs, catalogs = {}, {}
    for filter in imgs.keys():
        root_name = imgs[filter].split('.')[0]
        bkg_imgs[filter] = '{}_bkgsub.fits'.format(root_name)
        catalogs[filter] = '{}_.cat'.format(root_name)

        for test in [bkg_image[filter], catalogs[filter]]:
            if not os.path.exists(test):
                raise FileNotFoundError('{} not found! Are you sure you ran Source Extractor?'.format(test))

        if filter is detection_image:
            segmentation_map = '{}_segmap.fits'.format(root_name)
            if not os.path.exists(segmentation_map):
                raise FileNotFoundError('{} not found! Are you sure you ran Source Extractor?'.format(segmentation_map))

    output_file = '{}_catalog.fits'.format(cluster_name)
