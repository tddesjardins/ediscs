...
def show_fit_residuals(root='photz_v1.7.fullz', PATH='./OUTPUT/', savefig=None, adjust_zeropoints='zphot.zeropoint', fix_filter=None, ref_filter=None, get_resid=False, wclip=[1200, 3.e4], FITScat = '', filters='', outplot='modelresiduals.pdf'):
    """
    ...
    obsData=Table.read(FITScat).to_pandas()
...
#    tempfilt['efnu'][ok] *= zpfactors[ok]
    ...
    obs_sed=obs_sed/zpfactors
    resid = (obs_sed-tempfilt['fnu']) / obs_sed 
    ...
    nfil = len(filters)
    rauto = -2.5*np.log10(obsData['fRauto'].values)+23.9
    ...
    for x in range(nfil):
        good = np.where((obsData['sexflagR'] == 0) & (obsData['class_StarR'] < 0.3))
        plt.subplot(nfil, 1, x+1)
        data = resid[x,:]
        plt.plot(rauto,data,'k,',ms=1.)
        plt.plot(refx,refy,'k--')
        plt.plot(statx1,staty,'k',ls='dotted')
        plt.plot(statx2,staty,'k',ls='dotted')
        plt.axis([18.,26.,-0.2,0.2])
        statData = resid[x,np.where((rauto > 20.) & (rauto < 22.))]
        mean = bl(statData[np.where(np.abs(statData) < 0.2)])
        amean.append(mean)
        if mean < 0.:
            sign = r'$-$'
        else:
            sign=''
        spread = bs(statData[np.where(np.abs(statData) < 0.2)])
        aspread.append(spread)
        xpos=18.2
        plt.text(xpos,0.15,filters[x],fontsize=14)
        plt.text(xpos,0.10,'BW mean = '+sign+str(np.around(np.abs(mean),3)),fontsize=14)
        plt.text(xpos,0.05,'BW midvar = '+str(np.around(np.abs(spread),3)),fontsize=14)
        plt.xlabel(r'$m_{R,\ auto}$ / AB mag')
        plt.ylabel(r'$(F_{\nu,\ obs}-F_{\nu,\ mod})/F_{\nu,\ obs}$')
...
#    cwd = os.getcwd()+'/'
#    
#    if os.path.exists(cwd+'tweak_%s/tweak.dat' %(root)):
#        os.path.exists(cwd+'tweak_%s/tweak.dat' %(root))
#        tx, ty = np.loadtxt(cwd+'tweak_%s/tweak.dat' %(root), unpack=True)
...
#    pdb.set_trace()
#    np.savetxt(cwd+'tweak_%s/tweak.dat' %(root), np.array([xmfull, ymfull*ty_int]).T, fmt='%.5e')
#    
#    NT = len(param.templates)
#    fp = open(cwd+'tweak_%s/spectra.param' %(root),'w')
#    for i in range(NT):
...
#        ax.text(0.99, 0.99-0.04*ci*28./NF, '%s %3d %.3f' %(os.path.basename(param.filters[i].name).replace('_','_'), param.filters[i]#.fnumber, offsets[i]/ref_offset), transform = ax.transAxes, color=colors[i], alpha=0.8, horizontalalignment='right', va='top', fontsi#ze=font)
#
